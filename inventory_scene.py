...
def draw():
    ...
    # 배경 이미지 그리기 (이미지만 가로 100 늘림, 투명도 50%)
    if quest_bg_image:
        quest_bg_image.opacify(0.5)  # 투명도 50%
        quest_bg_image.draw(center_x, center_y, 600, box_height)  # 이미지는 600x600
        quest_bg_image.opacify(1.0)  # 원래대로 복구
    else:
    ...
    if font:
        ...
        # 볼드 효과: 텍스트를 여러 번 그려서 두껍게 보이게 함
        for dx in [0, 1]:
            for dy in [0, 1]:
                font.draw(center_x - 60 + dx, top - 40 + dy, title_text, title_color)
    ...
    if quest_manager:
        ...
        if len(quests) == 0:
            ...
            if font:
                ...
                for dx in [0, 1]:
                    for dy in [0, 1]:
                        font.draw(center_x - 80 + dx, center_y + dy, no_quest_text, color)
        else:
            # 퀘스트 목록 표시
            start_y = top - 80
            quest_spacing = 100
            ...
            for i, quest in enumerate(quests):
                ...
                if font:
                    ...
                    for dx in [0, 1]:
                        for dy in [0, 1]:
                            font.draw(left + 30 + dx, quest_y + dy, quest.title, title_color)

                    # 진행도 그리기 (볼드 효과)
                    progress_text = f"진행도: {quest.get_progress_text()}"
                    for dx in [0, 1]:
                        for dy in [0, 1]:
                            font.draw(left + 30 + dx, quest_y - 25 + dy, progress_text, progress_color)
